...
import { Table, TableBody, TableCell, TableFooter, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { Button } from "@/components/ui/button"
import { Search, Trash2, Plus, Edit2, Download } from "lucide-react"
import { Input } from "@/components/ui/input"
import Loader from "@/components/loading/loader"
import { useDebounce } from "@/hooks/use-debounce"
import { useSession } from "next-auth/react"
    ...
    <div className="p-8 flex w-full bg-gray-100 min-h-screen">
      <div className="w-11/12 mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <Button onClick={() => router.push("/orders/add-product")} className="bg-black text-white hover:bg-gray-800">
            <Plus className="mr-2 h-4 w-4" />
            Add Order
          </Button>
        </div>
                  ...
                  <TableBody className="text-xl">
                    {Array.isArray(orders) &&
                      orders.map((order) => (
                        <TableRow key={order._id} className="hover:bg-gray-50">
                          <TableCell className="font-medium">
                            {order._id ? order.trackOrderId : "Unknown Client"}
                          </TableCell>
                          {/* Client Name */}
                          <TableCell className="font-medium">
                            {order.clientId ? order.clientId.name : "Unknown Client"}
                            ...
                            {order._id && order._id.length > 0 ? (
                              ...
                              >
                                {order.croppedImage ? (
                                  <>
                                    <Image
                                      src={order.croppedImage || "/placeholder.svg"}
                                      alt="Product"
                                      width={100}
                                      height={60}
                                      className="object-cover w-full h-full transition duration-300 group-hover:opacity-60"
                          ...
                          <TableCell
                            className={
                              order.paymentMethod === "COD" ? "text-black font-medium" : "text-black font-medium"
                            }
                          >
                            {order.paymentMethod === "COD" ? "COD" : order.paymentMethod}
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="flex px-1 items-center space-x-2">
                              <span
                                className={
                                  order.paymentStatus === "pending" || order.paymentStatus === "cancel"
                                    ? "font-medium text-red-500"
                                    : "text-green-500 font-medium"
                                }
                              >
