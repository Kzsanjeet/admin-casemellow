...
interface DeleteFormProps {
  ...
  brandId: string; // ID of the brand to delete
  onDeleteSuccess: (isDeleted: boolean) => void; // Callback after successful deletion
}

const DeleteForm = ({ onClose, brandId, onDeleteSuccess}: DeleteFormProps) => {
  ...
  const handleDeleteFunc = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_LOCAL_PORT}/brands/delete-brand/${brandId}`,
        {
          method: "DELETE",
          headers: {
