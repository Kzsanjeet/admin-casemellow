...
interface DeleteFormProps {
  ...
  customizeId: string; // ID of the brand to delete
  onDeleteSuccess: (isDeleted: boolean) => void; // Callback after successful deletion
}

const CustomizeDeleteFrom = ({ onClose, customizeId, onDeleteSuccess}: DeleteFormProps) => {
    ...
    const handleDeleteFunc = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_LOCAL_PORT}/customize/delete/${customizeId}`,
          {
            method: "DELETE",
            headers: {
        ...
        if (response.ok) {
          ...
          throw new Error(data.message || "Failed to delete the Model.");
        }
      } catch (error: any) {
      //   setError(error.message || "Something went wrong.");
        toast.error(error.message || "Failed to delete the Model.");
      } finally {
        setLoading(false);
...
export default CustomizeDeleteFrom
